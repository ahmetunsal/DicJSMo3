...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ekle", "add"].includes(args[0])) {
      if (!args[1]) return message.channel.error(message, "Bir tag belirt ve tekrar dene.");
      const filtered = message.guild.members.cache.filter((x) => x.user.username.includes(args[1]) && !x.roles.cache.has(conf.roles.karantina));
      ...
      message.channel.send(embed.setDescription(`Başarıyla ${args[1]} tagı yasaklı taglar arasına eklendi ve ${filtered.size} kişi cezalıya atıldı!`));
      filtered.forEach(async (x) => {
        await x.setRoles(conf.roles.karantina);
        ...
        if (conf.dmMessages) x.send(`**${message.guild.name}** sunucusunda, kullanıcı adınızda yasaklı tag (\`${args[1]}\`) bulundurduğunuz için cezalıya atıldınız!`).catch(() => {});
        message.guild.channels.cache.get(conf.channels.YasaklıTag).send(`${x.toString()}, kullanıcı adınızda yasaklı tag (\`${args[1]}\`) bulundurduğunuz için cezalıya atıldınız!`);
...
Jail Sebebi: \`Kullanıcı adında yasaklı tag (${args[1]}) bulundurmak.\`
          `);
        message.guild.channels.cache.get(conf.logs.jail).send(log);
      });
      ...
      filtered.forEach(async (x) => {
        ...
        await x.setRoles(conf.roles.unregister);
        if (conf.dmMessages) x.send(`**${message.guild.name}** sunucusunda, ${args[1]} tagı yasaklı taglar arasından kaldırıldı ve cezanız açıldı çık gir yaparak kayıt olabilirsiniz.`).catch(() => {});
...
Jailin Kaldırılma Tarihi: \`${moment(Date.now()).format("LLL")}\`
      ...
      if (!data || data && !data.tags.length) return message.channel.send("Sunucuda herhangi bir yasaklı tag bulunmuyor!");
